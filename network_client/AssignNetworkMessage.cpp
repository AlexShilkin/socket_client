...
AssignNetworkMessage::AssignNetworkMessage(int16_t idClient)
{
	...
	if (packSize > 0) {
		...
		packSize = pack(bufferReturn, maskType(),
			...
			(int16_t)ASSING_NETWORK_MESSAGE,
...
NetworkMessage* AssignNetworkMessage::getNetworkMessage(char* data)
{
	int16_t clientId;
...
int16_t AssignNetworkMessage::getTypeMessage()
{
	return ASSING_NETWORK_MESSAGE;
}

int16_t AssignNetworkMessage::getIdClinet()
{
